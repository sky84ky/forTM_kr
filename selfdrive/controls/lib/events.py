...
EVENTS: Dict[int, Dict[str, Union[Alert, Callable[[Any, messaging.SubMaster, bool], Alert]]]] = {
  ...
  EventName.outOfSpace: {
    ET.PERMANENT: Alert(
      ...
      AlertStatus.normal, AlertSize.small,
      Priority.LOWER, VisualAlert.none, AudibleAlert.none, 0., 0., .2),
    ET.NO_ENTRY: NoEntryAlert("저장공간부족",
                              duration_hud_alert=0.),
  },
  ...
  EventName.soundsUnavailable: {
    ET.PERMANENT: NormalPermanentAlert("스피커가 감지되지 않습니다", "이온을 재부팅하세요"),
    ET.NO_ENTRY: NoEntryAlert("스피커가 감지되지 않습니다"),
  },
  ...
  EventName.calibrationInvalid: {
    ET.PERMANENT: NormalPermanentAlert("캘리브레이션 불확실", "위치 수정 후 재캘리브레이션"),
  ...
  EventName.lowMemory: {
    ...
    ET.PERMANENT: NormalPermanentAlert("메모리 매우 낮음", "EON을 재부팅하세요"),
  ...
  EventName.reverseGear: {
    ...
    ET.SOFT_DISABLE: ImmediateDisableAlert("기어 [R] 상태"),
  ...
  EventName.relayMalfunction: {
    ...
    ET.PERMANENT: NormalPermanentAlert("하네스 오작동", "연결상태를 점검하세요"),
  ...
  EventName.internetConnectivityNeeded: {
    ET.PERMANENT: NormalPermanentAlert("인터넷을 연결하세요", "활성화를위해 업데이트 확인이 필요합니다"),
