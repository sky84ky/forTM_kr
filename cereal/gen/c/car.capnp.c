...
void cereal_read_CarEvent(struct cereal_CarEvent *s, cereal_CarEvent_ptr p) {
	...
	s->resetVCruise = (capn_read8(p.p, 3) & 1) != 0;
}
void cereal_write_CarEvent(const struct cereal_CarEvent *s, cereal_CarEvent_ptr p) {
	...
	capn_write1(p.p, 24, s->resetVCruise != 0);
}
...
void cereal_read_CarState(struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	s->distanceToggle = capn_to_f32(capn_read32(p.p, 48));
	s->laneDepartureToggle = (capn_read8(p.p, 9) & 128) != 0;
	...
	s->readdistancelines = capn_to_f32(capn_read32(p.p, 44));
	s->lkMode = (capn_read8(p.p, 52) & 1) != 0;
	s->lcaLeft = (capn_read8(p.p, 52) & 2) != 0;
	s->lcaRight = (capn_read8(p.p, 52) & 4) != 0;
...
void cereal_write_CarState(const struct cereal_CarState *s, cereal_CarState_ptr p) {
	...
	capn_write32(p.p, 48, capn_from_f32(s->distanceToggle));
	capn_write1(p.p, 79, s->laneDepartureToggle != 0);
	...
	capn_write32(p.p, 44, capn_from_f32(s->readdistancelines));
	capn_write1(p.p, 416, s->lkMode != 0);
	capn_write1(p.p, 417, s->lcaLeft != 0);
	capn_write1(p.p, 418, s->lcaRight != 0);
...
void cereal_read_CarParams(struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	s->mdpsBus = (int8_t) ((int8_t)capn_read8(p.p, 80));
	s->sasBus = (int8_t) ((int8_t)capn_read8(p.p, 81));
	s->sccBus = (int8_t) ((int8_t)capn_read8(p.p, 82));
	s->autoLcaEnabled = (int8_t) ((int8_t)capn_read8(p.p, 83));
}
void cereal_write_CarParams(const struct cereal_CarParams *s, cereal_CarParams_ptr p) {
	...
	capn_write8(p.p, 80, (uint8_t) (s->mdpsBus));
	capn_write8(p.p, 81, (uint8_t) (s->sasBus));
	capn_write8(p.p, 82, (uint8_t) (s->sccBus));
	capn_write8(p.p, 83, (uint8_t) (s->autoLcaEnabled));
}
